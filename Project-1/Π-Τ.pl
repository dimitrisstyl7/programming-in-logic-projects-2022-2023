...
dateofbirth(Parent,Year1) :- Parent = person(_,_,date(_,_,Year2),_), Year2 < Year1.
salary(Parent,Salary1) :- Parent = person(_,_,_,works(_,Salary2)),  Salary2 < Salary1.
exists(Parent) :- getParent(Parent), dateofbirth(Parent,1950), salary(Parent,8000).